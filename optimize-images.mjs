...
for (const job of jobs) {
  ...
  const base = sharp(path.join(SRC, job.src))
...
{
  const meta = await sharp(path.join(SRC, '15.png')).metadata();
  await sharp(path.join(SRC, '15.png'))
...
{
  const meta = await sharp(path.join(SRC, '18.png')).metadata();
  await sharp(path.join(SRC, '18.png'))
